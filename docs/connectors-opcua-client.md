...
Set `SubscriptionSequentialPublishing = true` to process subscription messages sequentially at the cost of reduced throughput. This orders subscription messages against each other, but does not order them against local writes or transaction commits. An older notification can overwrite a newer locally committed value, including moving its tracked timestamp backward. Without a subsequent update or refresh, that older value can remain in the local model. This consistency defect is tracked in [#373](https://github.com/RicoSuter/Namotion.Interceptor/issues/373).

A successful transaction commit applies its accepted values locally before returning; it does not wait for subscription echoes. Consequently, client/server value equality is not proof that earlier notifications have drained. A differing notification that lands after the next transaction captures its old value can cause `SubjectTransactionConflictException`, even when that notification has an older or equal `SourceTimestamp`. An unchanged echo does not cause a value-based conflict. `TransactionFailureHandling.BestEffort` does not suppress conflicts; see [transaction conflict behavior](tracking-transactions.md#conflict-behavior). This also applies with sequential publishing enabled.
