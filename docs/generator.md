...
All diagnostics use the `Namotion.Interceptor` category. The tables below group them by their effect on generation. Unassigned IDs in each range are reserved for future rules.

Suppress a rule at the point of use with `#pragma warning disable NI0060`, or project-wide with `<NoWarn>NI0060;NI0061;NI0065</NoWarn>` in the project file. Both suppress generator diagnostics at `Error` severity as well as `Warning`. Each group explains what suppression leaves in place.

### Generation stops (NI0001–NI0019)
...
Suppressing one of these diagnostics silences the message, but generation still stops for the subject. Fix the underlying shape to obtain the generated members.

### Member emitted with advice (NI0020–NI0039)
...
Suppressing NI0020 accepts the attribute placement as declared. It only silences the advice; the generated code and property metadata are unchanged.

### Member skipped (NI0040–NI0059)
...
Suppressing NI0040 leaves the member skipped: no wrapper or metadata entry is emitted for it. Rename or reshape the member to make it supported.

### Generation continues with a semantic problem (NI0060–NI0079)
...
All rules in this group except NI0062 are errors, and none stops generation. Suppression leaves the semantic problem in place: an unreachable member (NI0060, NI0061, NI0065), a captured generated call (NI0063), or a hijacked interface slot (NI0064). Suppressing NI0062 accepts the fallback in which base-declared properties are not intercepted. When NI0061 concerns colliding members in interfaces you do not own, suppression can be the intended resolution because renaming those members is unavailable.
