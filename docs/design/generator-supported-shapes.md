...
**Compare resolved `DefaultProperties` blocks, not key lists.** The emitted sequence is own entries, then `Base.DefaultProperties`, then adopted interface defaults, resolved by `DistinctBy` keeping the first, so a changed entry does not show up as a key difference.
