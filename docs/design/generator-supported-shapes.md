...
The generator turns a partial class carrying `[InterceptorSubject]` into interception members. Most of that mapping is mechanical and self-explanatory from the code. A handful of decisions are not: they rest on the compiler's measured behaviour rather than on a reading of the language specification in the abstract. This document preserves that reasoning so it is not rediscovered by trial and error the next time one of these areas changes.
...
The generator emits code that reaches an interface default property or a class-declared explicit implementation by casting the subject to the declaring interface, for example `((IHuman)o).Gender`. Whether that cast-and-read compiles depends on whether the member is accessible from the generated code, which lives in the subject's own assembly, not the interface's.

The guard that answers it is `Compilation.IsSymbolAccessibleWithin(member, subjectType, throughType: accessorInterface)`, asked once per accessor. Every cheaper proxy for that question is wrong, each in a different assembly-boundary or accessor-level way, and the four below are worth knowing before touching `GetAccessorAccessibility` again.

1. **The property's own accessibility, checked against a hardcoded `Private`/`Protected`/`ProtectedAndInternal` list.** Roslyn reports `IPropertySymbol.DeclaredAccessibility` as `Private` for every explicit interface implementation, regardless of the implemented member's real visibility, because the CLR requires explicit implementations to be emitted private. Such a check therefore skips every explicit implementation, which silently reverts the fix for issue #428. The existing explicit-implementation tests fail on it.
2. **Exempting explicit implementations from the check entirely.** An explicit implementation of a `protected` interface member is then kept and emitted unconditionally, and the generated code fails with CS1540, since a protected member is only reachable through a type that derives from its declaring type, which generated code never does.
3. **The implemented member's accessibility against the same hardcoded list.** This gets the protected case right, but a hand-rolled list only understands same-assembly accessibility. It misses a cross-assembly `internal` member (CS0122, since `internal` is only accessible with a matching `InternalsVisibleTo`) and accessor-level accessibility, such as `string Probe { get; private set; }`, where the setter alone is unreachable (CS0272).
4. **`IsSymbolAccessibleWithin` without `throughType`.** Passing `throughType` is load-bearing: without it the query answers "is this member reachable from the subject type at all", which for a protected member is true through inheritance, and the protected case regresses back to CS1540. With `throughType` it answers the question the generated code actually asks: "is this member reachable through a cast to this specific interface".

The per-accessor pair (`GetAccessorAccessibility` in `SubjectMetadataExtractor.cs`) exists because a getter and setter can have independently weaker accessibility than the property itself; the emitter drops just the inaccessible half rather than the whole member.
...
**One helper, two call sites.** `ExtractInterfaceDefaultProperties` (interface default properties) and `CollectProperties` (class-declared explicit implementations, `string IFoo.Kind => ...` written directly on the subject class) both call the shared `GetAccessorAccessibility` rather than carrying a copy each. Two copies drift, and that drift is how the same CS1540 for a class explicitly implementing a protected interface member survived on the class-declared path while the interface-default path was being fixed: the fix did not touch the code path that also needed it.

The lesson generalises beyond this one guard: an accessibility rule that does not go through `IsSymbolAccessibleWithin`, or an equivalent compiler query, is testing a proxy for the real question. The history of `SubjectMetadataExtractor.cs` carries the individual steps as commits `d128ad6f` (the exemption), `6fd3f9ff` (the implemented member against the list), `7334ee06` (the symbol query that ships) and `2f55079f` (the shared helper).
...
Emitting the whole `IInterceptorSubject` block into every subject in a hierarchy is what issue #437 was: the most derived explicit implementation wins the interface map, so the generated constructor's `((IInterceptorSubject)this).Context.AddFallbackContext(context)` only ever populates the most derived `_context`. Every base class keeps a permanently null field and its generated accessors take the documented no-interception fast path, while `PropertyChanged` keeps firing, because the setter raises it directly rather than through the chain. The property value is right and the interceptors never run.

The block is emitted in one of two modes, chosen per class in `SubjectBaseContract.Resolve`.

**Root mode** emits everything: `_context`, `_properties`, the explicit `Context`, `Data`, `SyncRoot`, `Properties` and `AddProperties`, plus `GetInstanceProperties()` and the three helpers `GetPropertyValue`, `SetPropertyValue` and `InvokeMethod`. Those four are `protected` rather than `private` so a subject below can call them, which has consequences of its own, recorded under [The inherited helper surface](#the-inherited-helper-surface). `_context` and `_properties` stay private, because only the root's own members touch them.
...
else if a usable static DefaultProperties resolves through ancestor -> Root, report NI0062
else                                                                -> suppress, report NI0007
```
...
### The inherited helper surface

`GetPropertyValue`, `SetPropertyValue`, `InvokeMethod` and `GetInstanceProperties` are emitted `protected`, which is what lets a derived subject inherit them instead of re-emitting them. A private member would be invisible to a subclass and would hide nothing; four `protected` members are inherited by every class below the root, and a member that genuinely hides one of them is CS0108. `src/Directory.Build.props` sets `TreatWarningsAsErrors`, so in this repository, and in any consumer project that does the same, that is a build failure. This one is the compiler's diagnostic, not the generator's.

CS0108 therefore reaches a class the generator never scans. NI0063 covers a class that is itself `[InterceptorSubject]`, or one sitting between a subject and its subject ancestor. `SubjectBaseContract.Resolve` is only ever asked about a subject, so a plain hand-written subclass of a generated subject is not examined and no NI0063 can fire on it. That is not a hole, because CS0108 is loud rather than silent: the compiler names the file, the line and the hidden member. The consumer adds `new` where the hiding is intended, or renames the member. The one shape CS0108 does not cover is an overload that differs in signature, and there it is also harmless, because nothing generated calls the helpers from such a class.
...
This shape was built and verified to report all levels correctly. It is genuinely attractive: derived classes would stop listing `IInterceptorSubject`, CS0540 would stop constraining the design, the re-implementation hijack and the cross assembly rebuild gap below would both become structurally impossible, and `GetInstanceProperties()` would disappear from the emitted code and from the base class contract.

**The reason it is not used is not a measured cliff, and the record should not read as though it were.** `IInterceptorSubject.Properties` is read on every intercepted write through `PropertyReference.Metadata`, which is deliberately uncached (`PropertyReference.cs:20-29`, with callers in `IWriteInterceptor`, `LifecycleInterceptor` and `DerivedPropertyChangeHandler`), so the hook's virtual dispatch lands on a hot path and had to be measured rather than assumed. The measurement used two hand-written three level hierarchies reproducing the two dispatch shapes. Those mimics modelled a shape the generator never emits and so could never regress, which is why they are not in the tree; recover them from the history of `PropertiesDispatchShapeBenchmark.cs` if the question is reopened. What stands in `SubjectHierarchyBenchmark` is the chosen shape measured on real generated subjects, monomorphic and polymorphic, plus the `PropertyReference.Metadata` lookup itself.
...
The current design is kept with those numbers in hand rather than by default. The cost is small but it is paid by every subject forever, including the large majority that have no base class at all, while the hazard the hook would remove is caught at compile time by NI0064 for every consumer that recompiles. `AGENTS.md` ranks correctness above performance, so this is a close call rather than an obvious one, and anyone revisiting it should start from the numbers above rather than from the phrase "rejected on performance".

The numbers come from BenchmarkDotNet 0.15.5 on an Apple M4 Max under .NET 9.0.10, with `WarmupCount=5 IterationCount=15 LaunchCount=1`, and every configuration was run twice in separate processes so that the run to run spread supplies the noise floor rather than the within process error bars. Reproducing the comparison means restoring `PropertiesDispatchShapeBenchmark.cs` from history first, then running `dotnet run --project src/Namotion.Interceptor.Benchmark -c Release -- --filter "*PropertiesDispatchShapeBenchmark*"`; the chosen shape's own rows are `*SubjectHierarchyBenchmark*`.

## The risks that come with per hierarchy interception members

**Interface re-implementation can move a slot.** Because each subject re-lists `IInterceptorSubject`, the interface map is recomputed at that class, and a public member matching `Context`, `Data`, `SyncRoot` or `AddProperties`, or an explicit implementation of one of them, takes the slot from the root. `Context` is what makes the severity: the inherited helpers keep reading the root's `_context`, which nothing populates, so interception dies entirely and the unguarded `(IInterceptorExecutor)` casts in `DynamicSubjectFactory` and `RegisteredSubject` throw. `SyncRoot`, `Data` and `AddProperties` are milder than they look, since every product consumer of `SyncRoot` reads it through the interface and a hijack therefore redirects all of them to the same object rather than splitting the lock; they stay in the rule for consistency and because a user-owned lock object is still an aliasing hazard. NI0064 makes all of it a build error.

`Properties` is excluded from NI0064, because every subject emits its own explicit implementation of it, which always wins. A member declared in the same class as the explicit implementation it would displace loses to it for the same reason, which is why NI0063's scan stops before the ancestor that provides the inherited members, and why at and above that ancestor NI0064 reports only a member whose own base already implements the interface member.

**Cross assembly rebuild.** NI0064 fires where the derived subject is compiled, so a member added to a base assembly afterwards is not seen. The risk is narrower than a first reading suggests. All four of these have to hold:
...
Recompiling the consumer turns it into an NI0064 build error, so the exposure window is precisely "shipped, not rebuilt". The virtual hook above would make it structurally impossible.

**Interface evolution.** Any member added to `IInterceptorSubject` in future has to be evaluated for the same hijack question and added to NI0064's list, because derived subjects keep re-listing the interface.

**Writes before the context is published.** A derived class's field initializers run before the base constructor, so the context is null there and those writes take the fast path, as does anything in a constructor body that runs before `AddFallbackContext`. This is narrower than "construction time writes": `((IInterceptorSubject)this).Context` dispatches virtually, so a hand-written `Leaf(IInterceptorSubjectContext context) : base(context)` publishes the executor inside the base constructor, and a base declared property written afterwards in the leaf constructor body is intercepted. The hand written subclass contract depends on exactly this ordering.

**A suppressed ancestor produces calls to members that do not exist.** If an in source ancestor carries the attribute and is partial, but its own generation is suppressed by NI0002, NI0005 or NI0006, the derived class still selects derived mode and emits calls to members that were never generated. This is accepted: the build is already failing on the ancestor's own diagnostic and on CS9248 for its partial properties, so it adds noise to an already red build rather than hiding anything. NI0001 and NI0003 are not in that list, because `WillBeGeneratedInThisCompilation` already excludes a non-partial declaration and a record declaration.

## NI0063 and NI0064, the interception member capture rules

Both rules guard the members a derived subject inherits instead of emitting, and both fire only in derived mode, where the capturing member and the code it captures live in different classes.

**NI0063** fires when the subject, or any class between it and its subject ancestor, declares a member named `GetPropertyValue`, `SetPropertyValue`, `InvokeMethod` or `GetInstanceProperties`. The rule is deliberately over-approximate: name only, any member kind, no signature test, statics included. A precise rule would be both harder and unsound, because capture has two quiet routes. A `new` annotated member of the same shape captures the generated call with no compiler diagnostic at all, and an applicable overload with a different signature can win overload resolution without hiding anything. On an intermediate class the scan is restricted to members accessible from the subject, because a private member there neither hides nor binds and reporting it would be a pure false positive.

The rule is asymmetric between the two modes. A subject may legally declare `public void InvokeMethod(string name)` in root mode, where the signature differs from the generated helper, and the same source is an error in derived mode. The ground for leaving root mode unguarded is narrower than it first looks. An *identical* signature there is a hard CS0111, so that half really is covered by the compiler. A different signature is not covered by anything: a root subject declaring `protected bool SetPropertyValue(string, string, string, Action<IInterceptorSubject, string>)` alongside a `string` partial property compiles with zero diagnostics, wins overload resolution against the generated generic helper, and captures every write to that property. That residual case is a known gap rather than a guarded one. It is left open because the colliding member and the generated call are two halves of one class the author owns, so the fix is local and the shape is visible from the declaration, whereas in derived mode the capturing member and the code it captures live in different classes. `RaisePropertyChanged` is deliberately outside the rule: it is inherited whenever the base provides the notify members, so a `new` annotated override of it may well be deliberate.

**NI0064** fires in the same range, plus the subject ancestor and the classes above it under the condition below, for the four hijackable interface members `Context`, `Data`, `SyncRoot` and `AddProperties`. Two details of how it matches are worth knowing before the rule is narrowed. The public member clause matches only a genuine implicit implementation, comparing the member type, parameter types and ref kinds, and requiring the accessors to be publicly callable, so an ordinary `string Data` or a `bool AddProperties(...)` on a domain model is not reported. And the explicit implementation clause is reported on the subject itself as well, without a "declaring class is not a subject" qualifier: in derived mode the subject's generated half contains nothing but `IInterceptorSubject.Properties`, so a hand-written explicit `Context` on the user's half is not a conflict the compiler catches, it is the severe silent case.

A derived subject declaring `public object SyncRoot { get; }` is reported for the same reason: in derived mode that class emits no explicit implementation of its own to beat its public member, so the member takes the slot from the root.

NI0063 is scoped from the subject up to the nearest subject ancestor and excludes that ancestor, whose members are the ones the contract demanded of it. NI0064 is not bounded: it walks the whole chain up to `object`, because the nearest subject ancestor is exactly where a hand-written hijacker sits, and a second one further up would otherwise be invisible. Below that ancestor the report is unconditional. At it and above it, the member is reported only when the declarer's own base already implements the same member (`TakesSlotFromAbove`), which keeps the ordinary hand-written subject deriving from `object` quiet and cannot be asked below the ancestor, where an ancestor generated in the same compilation does not implement the interface as a symbol yet.

That condition carries no exemption for an explicit implementation, which would be dead weight: CS0540 forces the declaring class to list `IInterceptorSubject`, listing it makes that class the nearest subject ancestor and therefore always the contract provider, so such an exemption fires on every base class there is and hides the only form a hand-written ancestor can express. An `override` is skipped instead, since it occupies the slot it already had and displaces nothing.

Both rules take the nearest subject ancestor as the contract provider rather than locating one per member by re-running mode selection upward. It is the same class in every shape where the contract is satisfied as a whole, and it is simpler to reason about.

## Subject property precedence across a hierarchy

`EmitDefaultProperties` emits a precedence-ordered sequence, deduplicated by `GroupBy` so the first occurrence of each key wins:

1. the subject's own class-declared properties, including an explicit interface implementation declared in the subject's own class (highest);
2. everything inherited through `base.DefaultProperties`;
3. the subject's own adopted interface default implementations (lowest).

That order is the one a reader expects at every level: a derived subject's own declaration, along with its interception and its attributes, beats the entry the base class contributed for the same name. Concatenating a subject's own entries with its base's and letting `ToFrozenDictionary` resolve the duplicate key keeps the last one, which is the base, and inverts the precedence at every level.

An adopted interface default ranks last because it is a fallback. By the time a subject's own dictionary is built, tier 2 already carries whatever an ancestor contributed under that name; ranking the adopted default any higher would let a subject that declares nothing of its own re-inject the default over that ancestor's real property.

Only the interface-defaults tier is gated on being non-empty; tier 1 is emitted unconditionally, as an empty dictionary literal when the subject declares no properties of its own. A root subject, which has no base tier to concatenate against, stays a single dictionary with no `Concat` and no deduplication at all.

Deduplicating explicitly, keeping the first occurrence, is preferred to relying on `ToFrozenDictionary`'s duplicate handling for three reasons. For a subject with no interface defaults the two produce an identical key set in an identical first-occurrence order, so for the overwhelming majority of subjects the explicit form disturbs no frozen layout at all, whichever internal implementation `ToFrozenDictionary` happens to select. `ToFrozenDictionary`'s duplicate-key behaviour is not documented, so precedence should not rest on it. And the emitted code states the precedence rule directly, as a sequence of `.Concat` calls in precedence order, rather than leaving it implied by which entry a hash table decided to keep.

`GroupBy(pair => pair.Key).Select(group => group.First())` rather than the shorter `DistinctBy(pair => pair.Key)`, because this code runs in the consumer's compilation, not in this repository's. `DistinctBy` is .NET 6 or later, and a subject may be declared in a `netstandard2.0` assembly: the core library targets it, and the generator ships to `analyzers/dotnet/cs`, which applies to every consuming target framework. `ToFrozenDictionary` does **not** force .NET 6 the way it first appears to, because `System.Collections.Immutable` supplies the frozen collections down-level, so emitting `DistinctBy` breaks every derived subject in a `netstandard2.0` consumer with `CS1061` while the rest of the emission still compiles. Verified by building a `netstandard2.0` consumer with a base and a derived subject against the generator. `GroupBy` has the same first-wins semantics, since groups come out in first-occurrence order of their key and each group preserves source order.

`FrozenDictionary` does not preserve insertion order in general; it selects one of several internal implementations from the key set, and only some of those preserve order. Measured directly: eight keys of realistic length preserved insertion order, but fourteen same-length keys did not. Fourteen same-length property names is an ordinary generated model, so no part of this design, or any future one, may rest on `Properties` enumeration order. One narrower case is worth naming explicitly: for a derived subject with adopted interface defaults, the interface tier sitting at the end of the emitted sequence can change the enumeration order `FrozenDictionary` happens to produce, relative to what an earlier generator version produced for the same model. No ordering guarantee exists in either version, and nothing in the library depends on one, but a consumer silently relying on enumeration order would see it shift.

One policy governs the diagnostics in this area: fail loudly for anything the library cannot fully support, and prefer over-constraining a shape to hiding its defect. A shape whose defect is invisible from the declaration must not compile. NI0060, NI0061 and NI0065 are all `Error` under that policy.

NI0060 and NI0065 are not mutually exclusive: a subject can hide an ancestor subject's property that also happens to implement an interface, in which case both rules match the same declaration. Discriminating which one to report by asking "which class declared the colliding name" was rejected, because a plain class between two subjects can declare the name while contributing nothing to any `DefaultProperties`. `WhenAPlainClassBetweenTwoSubjectsDeclaresTheName_ThenNI0060IsReportedAndNI0065IsNot` in `DiagnosticTests.cs` is exactly that shape: the plain class declares the name, but the conflict belongs to the ancestor subject's adopted interface default, so NI0060's re-list remedy is the correct one and NI0065 must stay silent. The rule instead asks NI0065's own question directly, whether an ancestor **subject's** `DefaultProperties` already contributes the name, and reports NI0065 alone when it matches: it is the more severe failure, two intercepted backing fields sharing one key, and its remedy subsumes NI0060's. The names NI0065 reports on are handed to the NI0060 check regardless of whether NI0065 itself was suppressed with `#pragma`, so suppressing NI0065 for a property name also silences NI0060 for that same name; there is no way to accept the ancestor-displacement shape while still being warned that the interface slot did not move.

NI0065's own known limits, three of them, are recorded in [Known limitations of the diagnostics](#known-limitations-of-the-diagnostics) alongside the rest.

### Accessors belong to the overridden slot

Selecting the overriding declaration's metadata must not discard an accessor that C# still inherits. The extractor keeps `HasGetter` and `HasSetter` tied to the declaration, because the emitter uses them to implement partial properties. Separate `HasInheritedGetter` and `HasInheritedSetter` flags describe additional metadata accessors. They follow `OverriddenProperty` through generated subjects and plain intermediate classes, stop at the first corresponding accessor, and check accessibility through the leaf type. A lookup in an ancestor's `DefaultProperties` by name alone would be wrong when a plain intermediate class introduces a separate virtual slot with that name. The caller-facing rule is in [Virtual and Override Properties](../generator.md#virtual-and-override-properties).

### Interface collision checks use slot identity

An interface can explicitly override a default property from another interface. Both declarations then dispatch through the same implemented property, so treating their shared simple name as a collision would reject valid source. The extractor deduplicates the implemented property symbol before comparing names. `AllInterfaces` visits derived interfaces before their bases, retaining the most-derived declaration, and `SymbolEqualityComparer.Default` keeps different constructed generic interfaces distinct. Two different slots with one simple name still report NI0061, including when one of those slots has several explicit overrides.

### Why an interface property that narrows to `init` is dropped

Under the same policy, an interface property whose only reachable accessor is `init`, such as `{ protected get; init; }`, is dropped rather than kept as the phantom entry it would otherwise produce. Two consumer-visible things follow from that.

The property is **absent** from `IInterceptorSubject.Properties`, rather than present with both accessor lambdas null. `subject.Properties["X"]` throws `KeyNotFoundException` instead of returning an entry that does nothing observable, so code that indexes the key, or enumerates the dictionary expecting it, behaves differently with no opt-in and no compiler feedback. That is the same category of consumer-visible change as the precedence order itself.

The class-declared explicit-implementation form of the shape reports **NI0040** and skips the member. NI0040 is a `Warning`, and `src/Directory.Build.props` sets `TreatWarningsAsErrors` here as many consumer projects do, so in those projects the build fails. The remedy is the one the message names: declare a `get` or `set` accessor the generated half can reach, or drop the explicit implementation. NI0040's row in [the diagnostics table](../generator.md#diagnostics) lists this narrowing trigger separately from the "no reachable accessor" one, because an `init` accessor is reachable and still unusable.

## Known gaps

These are shapes the generator does not handle, and except where a bullet says otherwise, no diagnostic tracks them. None has an issue of its own, and each should get one if it turns out to matter in practice.

- **A `*WithoutInterceptor` method whose stripped name collides with an existing method on the class** emits a duplicate member and fails with CS0111. For example, declaring both `void Probe()` and `void ProbeWithoutInterceptor()` on the same subject: the generator strips the suffix and emits a second `void Probe()` wrapper, which the compiler rejects as a duplicate. The generator does not currently check for this collision before emitting the wrapper. The one case it does check is a collision with the names the generated half occupies, which is reported as NI0040 and the wrapper is not emitted, because there the wrapper captured the generated call, or an interface slot, instead of colliding with it. That check is name only, at any arity, with no exemptions. Two attempts to narrow it each let a capture through: comparing the parameter count ignored that `InvokeMethod` ends in `params object?[]`, so the emitted call sites span every arity from two upward and an overload applicable in normal form beats the helper, which needs the expanded form; and exempting `Context`, `Data` and `SyncRoot` treated "explicit interface property" as a property of the name rather than of the base, which is false for a hand-written base that satisfies the contract with public members, where the wrapper is a CS0108 in the generated file. The price is a false positive on a legitimate wrapper, `GetPropertyValueWithoutInterceptor(string, string)` and `InvokeMethodWithoutInterceptor(string, object?[])` among them, which is loud, actionable and fixed by renaming, where a capture is none of the three.
- **A class-declared init-only property still yields a metadata entry with both accessor lambdas null.** `public string OnlyInit { init { } }` is legal C# and reaches the emitter with neither an emittable getter nor an emittable setter, because the guard that drops the shape sits inside the explicit-interface-implementation branch of `CollectProperties`. The key exists and reading or writing it through the metadata does nothing observable. It is deliberately left alone rather than widened into another consumer-visible change alongside the interface-side drop above.

### Mixed generator versions across assemblies

A hierarchy spanning two assemblies built by different generator versions has two distinct failure modes, one of them diagnosed and one not.

- **A base assembly built before the interception members became shared emits its helpers `private`.** A private helper is invisible to a subclass, so the base fails the contract, the leaf falls back to root mode and reports NI0062. NI0062 is a `Warning`, so it fails the build only where warnings are errors, and rebuilding the base assembly against the current package version clears it, which is what the message says.
- **A base assembly built before the precedence fix keeps the inverted precedence baked into its own `DefaultProperties`.** The precedence order is resolved where a class is compiled, not where it is consumed: such a base resolved its own `DefaultProperties` at its own compile time, in the inverted order, and that resolved dictionary is what a leaf recompiled against the current generator inherits. Recompiling only the leaf cannot repair the hierarchy; the whole chain has to be rebuilt. No diagnostic detects this, because from the leaf's own compilation the base's `DefaultProperties` is just a value of the expected type, and nothing distinguishes a correctly ordered one from a stale one.

### Hierarchy gaps

None of these comes from the per hierarchy interception members; each reproduces without them. They are recorded rather than fixed because each needs a decision of its own, and two of them would change metadata for every existing subject and so want their own regeneration evidence.
...
- **A plain intermediate class that lists an interface deriving from `IInterceptorSubject`** stops the ancestor walk, because `DeclaresInterceptorSubject` is satisfied by the declaration alone. The contract then fails and, when the real root is in the same compilation, `HasUsableDefaultProperties` cannot see the root's not yet generated `DefaultProperties` either, so the shape falls through to NI0007 rather than to the NI0062 fallback the mode selection ladder implies. Cross assembly the same shape works, because the symbols exist. A domain interface on an abstract intermediate is an ordinary modelling shape, and NI0007's remedy text does not fit it.
- **`DefaultProperties` is emitted by every subject but is not in the hiding table.** A plain base that declares a member of that name gets CS0108 in the generated file, because `EmitDefaultProperties` decides `new` from "there is a subject ancestor" rather than from the hiding rule the four accessor helpers use.
- **Hiding `RaisePropertyChanged` silently swallows change notifications.** See [Hierarchy Hazards](../generator.md#hierarchy-hazards) in the user documentation for the mechanism. NI0063 deliberately excludes that name because redirecting it can be intentional and a `new` member that calls `base` is legitimate, which cannot be distinguished without dataflow. Two independent reviews found this, so if it is ever reconsidered, a `Warning` gated on the single string parameter overload is the shape to reach for.

## Why the test strategy has three layers

Issue #428 shipped because the existing tests asserted on generated **text**: `Assert.Contains(@"""Status""")` passes on code that can never compile. A wrong string embedded in a larger wrong string still contains the substring being asserted. Regression tests for the reported shape are not enough on their own; the strategy has to be one that cannot pass on broken output by construction.

1. **Verify snapshots of the full generated source**, using Verify.Xunit's `.verified.txt`/`.received.txt` workflow. This catches unintended output changes, but only for the eighteen shapes that have a snapshot (ordinary, virtual, override, inheritance, accessor visibility, nesting, interface-default properties, and the two precedence tiers). Several supported shapes have none: explicit interface implementations, non-public subjects, subjects nested in a record, struct or interface, and `in` / `ref readonly` wrapper parameters. Layers 2 and 3 below are what cover those instead. A snapshot can also itself capture code that does not compile, so even for the shapes it does cover, it is necessary and not sufficient.
...
The first line is why the root is never hijacked by its own member, and the last is why NI0064 walks the chain instead of inspecting only the subject.

**A public member whose signature does not match does not take the slot, and produces no diagnostic.** `public object Data { get; }` on a derived subject leaves `((IInterceptorSubject)x).Data` resolving to the base's explicit implementation, with no CS0738. This is why NI0064 matches against the real interface member instead of by name: an ordinary property called `Data` or `Context` is harmless.
...
**Hiding is by signature for methods and by name for every other member kind, and it is not staticness sensitive.** A `new static` method hides an inherited instance method of the same name, and generated code calling that name by simple name binds to it. That is why NI0063 does not skip statics.
...
This is why NI0060's remedy is re-listing the interface, and it is the same rule that forces every generated subject to re-list `IInterceptorSubject`.
...
**`INamedTypeSymbol.GetMembers(name)` returns declared members only.** There is no inheritance flattening in the symbol API, and an interface default implementation is a member of the interface, not of the class that adopts it. This is what keeps NI0065 off the shape the precedence feature exists to support.

**`#pragma warning disable NIxxxx` suppresses a generator diagnostic at `Error` severity, not only at `Warning`.** Verified against this generator. It is the only escape hatch a consumer has from NI0060, NI0061 and NI0065, and the repository's own tests depend on it.

**An abstract property can be displaced, but only through a plain intermediate class.** Hiding an abstract member directly is `CS0534` in a concrete class and `CS0533` in an abstract one, and no concrete class can then derive from the latter. A plain class that overrides the member, `sealed override` in particular, makes a `new` declaration below it the only one C# permits, and the whole hierarchy compiles with no diagnostic. An abstract property also cannot be `partial` (`CS0750`). This is why NI0065 does not filter abstract candidates.

## Known limitations of the diagnostics

Most of these surface as a compile error. Some do not, and are worth knowing individually: a base that satisfies every symbol check NI0007 performs can still be behaviourally wrong, described below; in root mode a differently signatured helper captures the generated call with no diagnostic at all, as [NI0063 and NI0064](#ni0063-and-ni0064-the-interception-member-capture-rules) above records; and two of NI0065's own limits below are the same shape, a real displacement that the ancestor scan does not see at all.
...
**NI0063 is deliberately name-only, any member kind, no signature test.** It therefore reports members that could not actually capture the generated call. A precise rule would be unsound for the reasons recorded with the rule itself, and the false positives are on four names nobody chooses by accident.

**NI0063 and NI0064 fire only in derived mode.** A root mode subject that declares its own `GetInstanceProperties` gets a raw CS0111 inside the generated file rather than a diagnostic. Accepted because the name is new, so no existing source can carry it, and the error is loud.

**NI0007 verifies shape, not behaviour.** A base can satisfy every symbol check and still be wrong in three ways, listed under [Three things the compiler cannot check for you](../generator.md#three-things-the-compiler-cannot-check-for-you). The most damaging is a base whose helpers route through a different executor from the one its `Context` publishes: every check passes, and writes are then tracked by an executor nobody reads, so the model and the metadata disagree with no diagnostic anywhere.
...
**`WillBeGeneratedInThisCompilation` tests only that every declaration is a partial class.** An ancestor carrying the attribute whose own generation is suppressed for another reason, NI0002, NI0005, NI0006 or NI0007, therefore still puts its subclass into derived mode, which turns one actionable diagnostic on the ancestor into a handful of raw CS0103 and CS0117 errors on the subclass. Accepted because the build is already failing on the ancestor's own diagnostic, but it is noise, and modelling the remaining guards is a few cheap symbol and syntax checks on a symbol already in hand.

**NI0060, NI0063, NI0064 and NI0065 report at the subject's location, not the offending member's.** When the member lives on a plain class between the subject and the contract provider, the error points at the subject, the file actually containing the member is never named, and the diagnostic repeats once per subclass rather than once per member. NI0065 is the rule most likely to match several properties of one class, and it collapses all of them onto the one squiggle. Reporting at `member.Locations.FirstOrDefault(l => l.IsInSource)` with the current location as the fallback, and at the property declaration's own location for the two property rules, would fix all of it.

**NI0065's ancestor scan filters only indexers and statics, not accessibility.** `IsNeverASubjectProperty` is reused from the interface-default extraction, where it exists for that narrower purpose, so the predicate tests less than whether the ancestor really contributes the name to its own `DefaultProperties`. It produces no false positive today: `CollectProperties` applies no accessibility filter to an ordinary class-declared property, so even a private one is in that ancestor's `DefaultProperties` anyway, and the only properties it does skip for accessibility are explicit interface implementations, whose dotted symbol names the scan cannot see in the first place, which is the limitation below.

**NI0065 misses an ancestor's own explicit interface implementation.** Roslyn names an explicit interface implementation with its dotted form, so `ancestor.GetMembers("Bar")` does not find an ancestor's `IFoo.Bar` even though that ancestor's `DefaultProperties` key **is** `"Bar"`. Only one shape then falls through every rule: a derived **explicit implementation of a different interface** colliding on the simple name, where tier 1 replaces the ancestor's key with no diagnostic at all. Where the derived declaration is an ordinary property, NI0060 reports it anyway, because the ancestor's interface is in both `AllInterfaces` sets and `FindImplementationForInterfaceMember` resolves to the ancestor's explicit implementation, which is not the subject's own type.

**NI0065's scan is narrower than tier 2's own ancestor detection.** The scan is scoped to ancestors carrying `[InterceptorSubject]`, while the precedence tiers come from `FindNearestSubjectAncestor`, which also accepts a hand-written class declaring `IInterceptorSubject` directly. That narrowing is deliberate, but it is narrower than "no symbol query can reveal the base's contents", which holds only across an assembly boundary. An in-source hand-written base is visible to the compiler and still exempt.
...
**Compare resolved `DefaultProperties` blocks, not key lists.** The emitted sequence is own entries, then `Base.DefaultProperties`, then adopted interface defaults, resolved by `GroupBy` keeping the first, so a changed entry does not show up as a key difference.
...
## Shapes that broke without a test

Each of these six shapes was broken and none of them had a test, which is how each one survived. They are listed because a change in this area should expect the same: shapes that nobody has written down are where the bugs are.
