...
```
RootWithPublic : explicit wins    public member beside the explicit implementation loses
DerivedRelist  : base wins        the base's explicit implementation wins when nothing matches
DerivedHijack  : derived wins     a public member in a derived class takes the slot
LeafOverPlain  : middle wins      and so does one on a plain class in between
```

The probe was run against the hijackable interface member of the time, which the single-context work has since removed. The rule is a property of C# interface mapping rather than of that member, so it carries to the members that replaced it.
