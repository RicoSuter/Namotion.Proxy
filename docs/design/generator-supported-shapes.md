...
The current design is kept with those numbers in hand rather than by default. The cost is small but it is paid by every subject forever, including the large majority that have no base class at all, while the hazard the hook would remove is caught at compile time by NI0014 for every consumer that recompiles. `AGENTS.md` ranks correctness above performance, so this is a close call rather than an obvious one, and anyone revisiting it should start from the numbers above rather than from the phrase "rejected on performance".
...
`Properties` is excluded from NI0014, because every subject emits its own explicit implementation of it, which always wins. A member declared in the same class as the explicit implementation it would displace loses to it for the same reason, which is why NI0013's scan stops before the ancestor that provides the inherited members, and why at and above that ancestor NI0014 reports only a member whose own base already implements the interface member.
...
## NI0013 and NI0014, the interception member capture rules
...
One policy governs the diagnostics in this area: fail loudly for anything the library cannot fully support, and prefer over-constraining a shape to hiding its defect. A shape whose defect is invisible from the declaration must not compile. NI0005, NI0008 and NI0015 are all `Error` under that policy.

NI0005 and NI0015 are not mutually exclusive: a subject can hide an ancestor subject's property that also happens to implement an interface, in which case both rules match the same declaration. Discriminating which one to report by asking "which class declared the colliding name" was rejected, because a plain class between two subjects can declare the name while contributing nothing to any `DefaultProperties`. `WhenAPlainClassBetweenTwoSubjectsDeclaresTheName_ThenNI0005IsReportedAndNI0015IsNot` in `DiagnosticTests.cs` is exactly that shape: the plain class declares the name, but the conflict belongs to the ancestor subject's adopted interface default, so NI0005's re-list remedy is the correct one and NI0015 must stay silent. The rule instead asks NI0015's own question directly, whether an ancestor **subject's** `DefaultProperties` already contributes the name, and reports NI0015 alone when it matches: it is the more severe failure, two intercepted backing fields sharing one key, and its remedy subsumes NI0005's. The names NI0015 reports on are handed to the NI0005 check regardless of whether NI0015 itself was suppressed with `#pragma`, so suppressing NI0015 for a property name also silences NI0005 for that same name; there is no way to accept the ancestor-displacement shape while still being warned that the interface slot did not move.

NI0015's own known limits, three of them, are recorded in [Known limitations of the diagnostics](#known-limitations-of-the-diagnostics) alongside the rest.
...
The class-declared explicit-implementation form of the shape reports **NI0006** and skips the member. NI0006 is a `Warning`, and `src/Directory.Build.props` sets `TreatWarningsAsErrors` here as many consumer projects do, so in those projects the build fails. The remedy is the one the message names: declare a `get` or `set` accessor the generated half can reach, or drop the explicit implementation. NI0006's row in [the diagnostics table](../generator.md#diagnostics) lists this narrowing trigger separately from the "no reachable accessor" one, because an `init` accessor is reachable and still unusable.
...
- **A base assembly built before the interception members became shared emits its helpers `private`.** A private helper is invisible to a subclass, so the base fails the contract, the leaf falls back to root mode and reports NI0012. NI0012 is a `Warning`, so it fails the build only where warnings are errors, and rebuilding the base assembly against the current package version clears it, which is what the message says.
...
- **Hiding `RaisePropertyChanged` silently swallows change notifications.** See [Hierarchy Hazards](../generator.md#hierarchy-hazards) in the user documentation for the mechanism. NI0013 deliberately excludes that name because redirecting it can be intentional and a `new` member that calls `base` is legitimate, which cannot be distinguished without dataflow. Two independent reviews found this, so if it is ever reconsidered, a `Warning` gated on the single string parameter overload is the shape to reach for.
...
**NI0013 is deliberately name-only, any member kind, no signature test.** It therefore reports members that could not actually capture the generated call. A precise rule would be unsound for the reasons recorded with the rule itself, and the false positives are on four names nobody chooses by accident.
...
**NI0005, NI0013, NI0014 and NI0015 report at the subject's location, not the offending member's.** When the member lives on a plain class between the subject and the contract provider, the error points at the subject, the file actually containing the member is never named, and the diagnostic repeats once per subclass rather than once per member. NI0015 is the rule most likely to match several properties of one class, and it collapses all of them onto the one squiggle. Reporting at `member.Locations.FirstOrDefault(l => l.IsInSource)` with the current location as the fallback, and at the property declaration's own location for the two property rules, would fix all of it.

**NI0015's ancestor scan filters only indexers and statics, not accessibility.** `IsNeverASubjectProperty` is reused from the interface-default extraction, where it exists for that narrower purpose, so the predicate tests less than whether the ancestor really contributes the name to its own `DefaultProperties`. It produces no false positive today: `CollectProperties` applies no accessibility filter to an ordinary class-declared property, so even a private one is in that ancestor's `DefaultProperties` anyway, and the only properties it does skip for accessibility are explicit interface implementations, whose dotted symbol names the scan cannot see in the first place, which is the limitation below.

**NI0015 misses an ancestor's own explicit interface implementation.** Roslyn names an explicit interface implementation with its dotted form, so `ancestor.GetMembers("Bar")` does not find an ancestor's `IFoo.Bar` even though that ancestor's `DefaultProperties` key **is** `"Bar"`. Only one shape then falls through every rule: a derived **explicit implementation of a different interface** colliding on the simple name, where tier 1 replaces the ancestor's key with no diagnostic at all. Where the derived declaration is an ordinary property, NI0005 reports it anyway, because the ancestor's interface is in both `AllInterfaces` sets and `FindImplementationForInterfaceMember` resolves to the ancestor's explicit implementation, which is not the subject's own type.
