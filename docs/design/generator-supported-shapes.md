...
`DefaultProperties` used to concatenate a subject's own entries with its base's and let `ToFrozenDictionary` resolve any duplicate key by keeping the last one, which is the base. That inverted the precedence a reader expects at every level: a derived subject's own declaration, along with its interception and its attributes, was discarded in favour of the entry the base class contributed for the same name. `EmitDefaultProperties` now emits a precedence-ordered sequence instead, deduplicated by `GroupBy` so the first occurrence of each key wins:
...
Deduplicating explicitly, keeping the first occurrence, was chosen over relying on `ToFrozenDictionary`'s duplicate handling for three reasons. A subject with no interface defaults has an identical key set and first-occurrence order before and after this change, so the overwhelming majority of subjects never see their frozen layout change, whichever internal implementation `ToFrozenDictionary` happens to select. `ToFrozenDictionary`'s duplicate-key behaviour is not documented, so precedence should not rest on it. And the emitted code states the precedence rule directly, as a sequence of `.Concat` calls in precedence order, rather than leaving it implied by which entry a hash table decided to keep.

`GroupBy(pair => pair.Key).Select(group => group.First())` rather than the shorter `DistinctBy(pair => pair.Key)`, because this code runs in the consumer's compilation, not in this repository's. `DistinctBy` is .NET 6 or later, and a subject may be declared in a `netstandard2.0` assembly: the core library targets it, and the generator ships to `analyzers/dotnet/cs`, which applies to every consuming target framework. `ToFrozenDictionary` does **not** force .NET 6 the way it first appears to, because `System.Collections.Immutable` supplies the frozen collections down-level, so emitting `DistinctBy` would have broken every derived subject in a `netstandard2.0` consumer with `CS1061` while the rest of the emission still compiled. Verified by building a `netstandard2.0` consumer with a base and a derived subject against the generator. `GroupBy` has the same first-wins semantics, since groups come out in first-occurrence order of their key and each group preserves source order.

`FrozenDictionary` does not preserve insertion order in general; it selects one of several internal implementations from the key set, and only some of those preserve order. Measured directly: eight keys of realistic length preserved insertion order, but fourteen same-length keys did not. Fourteen same-length property names is an ordinary generated model, so no part of this design, or any future one, may rest on `Properties` enumeration order. This also covers a narrower case worth naming explicitly: for a derived subject with adopted interface defaults, moving the interface tier to the end of the emitted sequence can change the enumeration order `FrozenDictionary` happens to produce relative to earlier generator versions. No ordering guarantee ever existed before this change either, and nothing in the library depends on one, but a consumer who was silently relying on enumeration order would see it shift.

The diagnostic side of this work follows one policy: fail loudly for anything the library cannot fully support, and prefer over-constraining a shape to hiding its defect. A shape whose defect is invisible from the declaration must not compile. NI0005 and NI0008 move from `Warning` to `Error` under that policy, and NI0015 is new.
...
### Dropping an interface property that narrows to `init` changes two visible behaviours

Under the same policy, an interface property whose only reachable accessor is `init`, such as `{ protected get; init; }`, is now dropped rather than kept as the phantom entry it used to produce. Two consumer-visible things change with it.

The property is now **absent** from `IInterceptorSubject.Properties`, where it previously appeared with both accessor lambdas null. `subject.Properties["X"]` throws `KeyNotFoundException` instead of returning an entry that does nothing observable, so code that indexed the key, or enumerated the dictionary expecting it, changes behaviour with no opt-in and no compiler feedback. That is the same category of break as the precedence change itself.

The class-declared explicit-implementation form of the shape reports **NI0006** and skips the member. NI0006 is a `Warning`, and `src/Directory.Build.props` sets `TreatWarningsAsErrors` here as many consumer projects do, so a build that succeeded before now fails. The remedy is the one the message names: declare a `get` or `set` accessor the generated half can reach, or drop the explicit implementation. NI0006's row in [the diagnostics table](../generator.md#diagnostics) lists this narrowing trigger separately from the "no reachable accessor" one, because an `init` accessor is reachable and still unusable.
...
- **A class-declared init-only property still yields a metadata entry with both accessor lambdas null.** `public string OnlyInit { init { } }` is legal C# and reaches the emitter with neither an emittable getter nor an emittable setter, because the guard that drops the shape sits inside the explicit-interface-implementation branch of `CollectProperties`. The key exists and reading or writing it through the metadata does nothing observable. This predates the interface-side fix above and was deliberately left alone rather than widened into another break.
- **A base assembly compiled by an older generator keeps the inverted precedence baked into its own `DefaultProperties`.** The precedence fix changes what a subject emits, not what already shipped: a base class built before the fix resolved its own `DefaultProperties` at its own compile time, in the old, inverted order, and that resolved dictionary is what a leaf recompiled against the new generator inherits. Recompiling only the leaf cannot repair the hierarchy; the whole chain has to be rebuilt against the fixed generator. No diagnostic detects this mixed-version case, because from the leaf's own compilation the base's `DefaultProperties` is just a value of the expected type, and nothing distinguishes a correctly ordered one from a stale one.
