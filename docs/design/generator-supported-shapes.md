...
Under the same policy, an interface property whose only emittable accessor is `init`, such as `{ protected get; init; }`, is now dropped rather than kept as the phantom entry it used to produce. Two consumer-visible things change with it.
...
**NI0015's ancestor scan filters only indexers and statics, not accessibility.** `IsNeverASubjectProperty` is reused from the interface-default extraction, where it exists for that narrower purpose. An ancestor property the generator itself skipped as inaccessible still satisfies the predicate, so NI0015 can fire on a name the ancestor does not actually contribute to any `DefaultProperties`.
